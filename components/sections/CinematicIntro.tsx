...
export default function CinematicIntroOptimized() {
  ...
  const radialAccentRef = useRef<HTMLDivElement>(null)
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // -------------------------
      // Основной таймлайн появления элементов
      const intro = gsap.timeline({ defaults: { ease: 'power3.out' } })
      intro
        .fromTo(radialAccentRef.current, { scale: 0.95, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8 })
        .fromTo('#taglineTop', { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 }, '<0.1')
        .fromTo('#brand', { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 }, '<0.1')
        .fromTo('#bigTitleWrap', { y: 20, opacity: 0, scale: 0.98 }, { y: 0, opacity: 1, scale: 1, duration: 0.8 }, '<0.1')
        .fromTo('#threeSphereWrap', { scale: 0.9, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8 }, '<0.05')
      ...
      // -------------------------
      // Дыхание бирюзового радиального акцента
      if (radialAccentRef.current) {
        gsap.to(radialAccentRef.current, {
          scale: 1.05,
          opacity: 0.18,
          duration: 6,
          repeat: -1,
      ...
      // -------------------------
      // Плавное движение фоновых SVG кругов
      ...
      <div className="relative mx-auto max-w-[1920px] px-4 sm:px-6 md:px-12 lg:px-20 py-20">
        <div className="relative w-full rounded-[24px] border-2 border-cyan-500/90 p-8 sm:p-12 md:p-16 lg:p-20 overflow-hidden">
          ...
          <div className="relative z-10 text-center">
            ...
            <div id="brand" className="mb-3 text-sm font-semibold tracking-[0.25em] opacity-90">
              ZVERINACODE
            </div>
            ...
            <div id="bigTitleWrap" className="relative mx-auto inline-block max-w-md">
              <h1
                id="bigTitle"
                className="text-center font-extrabold uppercase leading-[0.86] text-cyan-500 tracking-[-0.02em] text-4xl sm:text-5xl md:text-6xl lg:text-7xl"
              >
                FRONTEND DEVELOPER
              </h1>
